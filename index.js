...
invariant(firstSleepIndex === -1 || firstWakeIndex < firstSleepIndex, 'The first "wake"" entry should come before the first "sleep" entry.')

function getTimeSlots(entries) {
	let working = true
	let currentSlot
	const endSlot = (slot, entry) => {
		slot.end = entry.date
		slot.duration = Math.floor((slot.end.getTime() - slot.start.getTime()) / 1000)
		invariant(slot.duration > 0, `slot.duration is negative for slot ${JSON.stringify(slot, null, 2)}`)
	}
	const makeNewSlot = (entry) => ({start: entry.date, type: working ? 'work' : 'personal'})
	const result = transform(entries, (slots, entry) => {
		switch (entry.action) {
			case WAKE:
			...
			case PERSONAL: {
				if (!working) return
				// invariant(working, `PERSONAL action executed while !working, looking at ${JSON.stringify(entry)}`)
				working = false
				endSlot(currentSlot, entry)
				slots.push(currentSlot)
				currentSlot = makeNewSlot(entry)
			...
			case WORK: {
				if (working) return
				// invariant(!working, `WORK action executed while working, looking at ${JSON.stringify(entry)}`)
				working = true
				endSlot(currentSlot, entry)
				slots.push(currentSlot)
				currentSlot = makeNewSlot(entry)
...
const time = (date) => moment(date).utcOffset(1).format('h:mma')
const date = (date) => moment(date).utcOffset(1).format('ddd, D.M')
